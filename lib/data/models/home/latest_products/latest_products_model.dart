...
class LatestProductsModel extends LatestProductsEntity {
  ...
  static List<LatestProductsModel> getLatestProductsFromList(
      {required List<dynamic> list}) {
    List<LatestProductsModel> newList = [];
    for (var element in list) {
      newList.add(LatestProductsModel.fromJson(json: element));
    }
    return newList;
  }

  factory LatestProductsModel.fromJson({required Map<String, dynamic> json}) {
  ...
  static final dummyData = [
    LatestProductsModel(
      ...
      subCategory: CategoriesModel(
          ...
          createdAt: "2023-10-12 01:25:02"),
      details:
          "يجمع معجون الأسنان الجل بالمينثول المنعش بين فعالية 3 مكونات قوية: جل التطهير وغسول الفم المضاد للبكتيريا والكريستالات دقيقة اللمعان للحفاظ على حماية أسنانك وانتعاشها. يمنع معجون أسنان كلوس اب مع غسول الفم بالزنك النشط تسوس الأسنان ويحافظ على انتعاش النفس لمدة تصل إلى 12 ساعة. يتميز معجون الأسنان بنعناع المينثول هذا بغسول الفم المضاد للبكتيريا لنفس منعش يدوم طويلاً. حارب رائحة الفم الكريهة وتسوس الأسنان مع معجون الأسنان الجل هذا ذو الرائحة المنعشة.",
      salesLimit: 10,
      price: 24,
      unit: "قطعة",
      weightUnit: 1,
...
class LatestProductsModel {
  ...
  factory LatestProductsModel.fromJson({required Map<String, dynamic> json}) {
    return LatestProductsModel(
      ...
      details: json['details'],
      salesLimit: json['sales_limit'],
      price: json['price']?.toDouble(), // Convert to double if not null
      unit: json['unit'],
      weightUnit: json['weight_unit'],
      priceWeightUnit: json['price_weight_unit']
          ?.toDouble(), // Convert to double if not null
      isOffer: json['is_offer'] ?? false,
      isActive: json['is_active'],
      offerType: json['offer_type'],
      offerValue:
          json['offer_value']?.toDouble(), // Convert to double if not null
      offerStartDate: json['offer_start_date'],
      offerEndDate: json['offer_end_date'],
      oldPrice: json['old_price']?.toDouble(), // Convert to double if not null
      isFavorite: json['is_favorite'],
    );
  }
